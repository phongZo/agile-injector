...
namespace AgileInjector
{
    public static class Injector
    {
        ...
        // Load a module WITHOUT running its DllMain (DONT_RESOLVE_DLL_REFERENCES). Used only to read an
        // export's RVA locally. The webcam DLL self-activates in DllMain (spawns threads + installs global
        // hooks); a plain LoadLibraryW + FreeLibrary of it inside the injector crashed AgileInjector.exe
        // (0xC0000005 in ntdll) under toggle-spam. Mapping it as no-DllMain avoids that entirely.
        ...
        {
            IntPtr hLocal = LoadLibraryExW(dllFullPath, IntPtr.Zero, DONT_RESOLVE_DLL_REFERENCES);
            if (hLocal == IntPtr.Zero)
            {
                int errLL = Marshal.GetLastWin32Error();
                DebugLog.WriteLine($"[Inject][ERROR] LoadLibraryExW(local) failed for RVA compute. {errLL} ({new Win32Exception(errLL).Message})");
                return IntPtr.Zero;
            }
            try
            {
